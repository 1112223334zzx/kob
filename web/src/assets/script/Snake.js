...
export class Snake extends AcGameObject {
    constructor(info, gamemap) {
        ...
        this.status = "static";  // static表示静止，move表示正在移动，die表示死亡
    ...
    check_tail_increasing() { //检测当前回合是否增加蛇的长度  规则：前十个回合每次都添加长度，接下来每三个回合加1,
    ...
    update_move() {
        ...
        if (distance < this.eps) {  // 走到目标点了
            ...
            if (!this.check_tail_increasing()) {
                ...
                tail.x += move_distance * tail_dx / distance; //这里不需要重新计算，因为蛇头到蛇头目标距离与蛇尾到蛇尾目标位置距离相等
